...
{
    public function read(PageClassification|WebsiteAudit|null $classification): array
    {
        $audit = $classification instanceof WebsiteAudit;
        $result = ['status' => 'unavailable', 'emails' => [], 'links' => [], 'scanned_at' => $audit ? $classification->finished_at : $classification?->classified_at];
        $metadata = $audit ? ($classification->summary['contact_scan'] ?? []) : ($classification?->classification_metadata ?? []);
        ...
        foreach (array_slice($pages, 0, $maxPages) as $page) {
            ...
            foreach (array_slice($emails, 0, $maxEmails) as $candidate) {
                ...
                $result['emails'][] = [
                    ...
                    // Audit timestamps are generated by the runner, never read from website HTML.
                    'observed_at' => $audit ? ($page['observed_at'] ?? null) : null,
                ];
            }
